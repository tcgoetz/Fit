...
class HeartRateVarianceField(FloatField):
    """A FIT file message field holding a heart rate variance measurement. RMSSD value (scale 128)"""
...
class PressureField(IntegerField):
...
class FlowRateField(IntegerField):
    """flow rate measure for diving?."""
...
class EnhancedRespirationRateField(FloatField):
    """A respiration rate measurment in breaths per minute."""

    _name = 'enhanced_respiration_rate'
    _units = 'brpm'
    _scale = 100.0
...
class HeadingField(IntegerField):
    """Direction."""
